...
for curclass = 1:r.nclass
    ...
    [~, r.FullModel{curclass}] = TRAINFUNC(r.Y{curclass}, r.BinaryLabel{curclass}, 1, Ps{curclass});    
    if r.PreSort
    ...
    ts = zeros(size(r.T{1},1),1);
     for curclass=1:numel(Y)
         if isempty(r.FullModel{curclass})
            [~, r.FullModel{curclass}] = TRAINFUNC(r.Y{curclass}, label{curclass}, 1, Ps{curclass});    
         end
...
r.AdRef = rfe_algo_settings_adaptive(RFE);
