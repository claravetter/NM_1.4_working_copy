...
% IO            :       Data input/output structure
% mess          :       Message structure containing sanity check results
% varind        :       Current modality to work on
% 
% Output:
% -------
...
switch datasource
    ...
    case {'spm','nifti','surf'}
        ...
        if ~oocvflag
            ...
            if ~strcmp(bmask_str,na_str) && ~exist(brainmask,'file') && ~isfield(IO,'Vmvol')
                bmask_act = 'Update';
                mess = GenerateMessageEntry(mess, sprintf('ERROR: Space-defining defining image could not be found in %s!\nUpdate path to point to the correct image!',brainmask));
                mn_groupmode = sprintf('%s space-defining image [ %s ]|', bmask_act, bmask_str);
                mn_act = [mn_act 'def_spacedefimg']; mn_str = [mn_str mn_groupmode];
            end
            if isfield(IO,'Pw') && ~exist(IO.Pw,'file')
                mn_weightimg_str = sprintf('Yes (%g images specified)',size(IO.Pw,1));
                mn_weightimg = sprintf('Update additional weighting image(s) [ %s ]|',mn_weightimg_str);
                mn_act = [ mn_act 'sel_weightimg' ]; mn_str = [ mn_str mn_weightimg ];
            end
        end
        ...
        if n_samples && ~strcmp(brainmask, na_str) && ( strcmp(datasource,'nifti') || strcmp(datasource,'surf'))
            ...
            if strcmp(modeflag, 'classification') && ( ~oocvflag || ( oocvflag && IO.labels_known ))
                mn_files = sprintf('Map image files to samples [ %s ]|', filestr);       
            else
                        ...
                        if ~strcmp(IO.M_edit,'')
                            ...
                            switch groupmode 
                                ...
                                case 4
                                    ...
                                    if isfield(IO,'sheet') && ~strcmp(IO.sheet, na_str); mess = GenerateMessageEntry(mess, sprintf('* Source sheet defined'), false,'blue'); end
                            ...
                            if isfield(IO,'cellcols') && any(IO.cellcols)
                                mn_uniquelim_edit = sprintf('Specify maximum # of unique values in non-numeric variables for dummy coding [ %g ]|', IO.uniquelim); 
                                mn_act = [ mn_act 'def_uniquelim' ]; mn_str = [mn_str mn_uniquelim_edit];
                            end
...
switch act
    ...
    case 'sel_weightimg'
        if ~oocvflag
            ...
            for i=1:size(IO.Pw,1)
                [~,nx,ex] = fileparts(deblank(IO.Pw(i,:))); 
                tdir = nk_DirSelector(sprintf('Specify directory of weighting image %g',i));
                tPw = fullfile(tdir,[nx ex]);
                if exist(tPw,'file')
                    IO.Pw(i,:)=tPw;
                    switch IO.datasource
                        case {'nifti','spm'}
                            IO.Vw(i) = spm_vol(IO.Pw(i,:));
                        case 'surf'
                            IO.Vw(i) = SurfaceReader(IO.Pw(i,:));
                    end
                end
            end
        end
    ...
    case 'def_uniquelim'
        IO.uniquelim = nk_input('Define max number of unique values in non-numeric variables allowed for dummy coding?',0,'i', IO.uniquelim);
    ...
    case 'm_inspect'
        ...
        switch groupmode    
            ...
            case {3,4}
                ...
                col_cases = strcmp(IO.t_Y.Properties.VariableNames,case_edit);
                col_label = RetCellInCellsIndex(IO.t_Y.Properties.VariableNames, label_edit);
                M = IO.t_Y(:, ~col_cases & ~col_label );
                if isfield(IO,'cellcols') && any(IO.cellcols)
