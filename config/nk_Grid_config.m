...
if ~defaultsfl 
    ...
    if prod(n_pars) * nP > 1
        ...
        menustr = sprintf('%s|Choose optimization method [ %s ]', menustr, OptMode.type); menuact = [menuact 200];

        switch OptMode.type
            case 'simanneal'
                T_str = sprintf('Define initial temperature [ %g ]', OptMode.SimAneal.T);
                max_jump_str = sprintf('Define max initial step size [ %g ]', OptMode.SimAneal.max_initial_jump);
                alpha_str = sprintf('Define cooling factor [ %g ]', OptMode.SimAneal.alpha);
                random_jump_prob_str = sprintf('Define random jump probability (0-0.5) [ %g ]', OptMode.SimAneal.random_jump_prob);
                max_iter_str = sprintf('Define maximum number of iterations [ %g ]', OptMode.SimAneal.max_iter);
                reheating_iter_str = sprintf('Define number of iterations without performance changes until reheating takes place [ %g ]', OptMode.SimAneal.reheating_iter);
                reheating_alpha_str = sprintf('Define re-heating factor [ %g ]', OptMode.SimAneal.reheating_alpha);
                menustr = sprintf('%s|%s|%s|%s|%s|%s|%s|%s', menustr, T_str, max_jump_str, alpha_str, random_jump_prob_str,max_iter_str, reheating_iter_str, reheating_alpha_str); 
                menuact = [menuact 201:207];
            case 'bayesopt'
    ...
    switch act
        ...
        case 11
            ...
            switch NodeSelect.mode
                ...
                case 3 
                    % This option is not implemented yet in in nk_ModelNodeSelector!!!
                case 4
        ...
        case 200
            hyperopt_defs= {'brute-force','simanneal','bayesopt'};
            def_hyperopt = find(contains(hyperopt_defs, OptMode.type));
            OptMode.type = char(nk_input('Define hyperparameter optimization mode', 0, 'm', ...
                'Brute-force optimization|Simulated annealing|Bayesian hyperparameter optimization', ...
                hyperopt_defs, def_hyperopt));
        case 201
            OptMode.SimAneal.T = nk_input('Define starting temperature', 0, 'i', OptMode.SimAneal.T);
        case 202
            OptMode.SimAneal.max_initial_jump = nk_input('Define initial jumping step size (number of hyperparameter combinations)', 0, 'i', OptMode.SimAneal.max_initial_jump);
        case 203
            OptMode.SimAneal.alpha = nk_input('Define cooling factor ''alpha''', 0, 'e', OptMode.SimAneal.alpha);
        case 204
            OptMode.SimAneal.random_jump_prob = nk_input('Define random jump probability', 0, 'e', OptMode.SimAneal.random_jump_prob);
        case 205
            OptMode.SimAneal.max_iter = nk_input('Define maximum number of iterations', 0, 'i', OptMode.SimAneal.max_iter);
        case 206
            OptMode.SimAneal.reheating_iter = nk_input('Define number of iterations without performance change until re-heating takes place', 0, 'i', OptMode.SimAneal.reheating_iter);
        case 207
            OptMode.SimAneal.reheating_alpha = nk_input('Define reheating factor', 0, 'e', OptMode.SimAneal.reheating_alpha);
        case 210
            OptMode.Bayes.max_iter = nk_input('Define maximum number of iterations', 0, 'i', OptMode.Bayes.max_iter);
        case 211
            OptMode.Bayes.max_iter_no_change = nk_input('How many interations without change are allowed until algorithm aborts?', 0, 'i', OptMode.Bayes.max_iter_no_change);
        case 212
            OptMode.Bayes.num_seed = nk_input('How many seed iterations should be performed?', 0, 'i', OptMode.Bayes.num_seed);
        case 213
            kernelfunc_defs= {'exponential','ardexponential','squaredexponential','ardsquaredexponential','matern32','ardmatern32','matern52','ardmatern52'};
            def_kernelfunc = find(contains(kernelfunc_defs, OptMode.type));
            OptMode.Bayes.kernel_function = char(nk_input('Select kernel function', 0, 'm', ...
                ['Exponential kernel|', ...
                 'Exponential kernel with automatic relevance detection (ARD)|',...
                 'Squared-exponential kernel|',...
                 'Squared-exponential kernel + ARD|', ...
                 'Matern kernel with parameter 3/2|', ...
                 'Matern kernel with parameter 3/2 + ARD|', ...
